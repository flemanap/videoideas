// =============================================
// SUPABASE EDGE FUNCTION - DAILY SCHEDULER
// =============================================
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const profile of profiles) {
      ...
      console.log(`Generated ${ideas.length} ideas for ${profile.name}`)
    }
    ...
    const totalIdeas = Object.values(ideasPerProfile).reduce((a, b) => a + b, 0)

    return new Response(
      JSON.stringify({
        success: true,
        message: `Daily generation complete: ${totalIdeas} ideas created`,
...
5. Include a description that works for ALL platforms (YouTube, TikTok, Instagram, X)
6. Include exactly 4 hashtags
...
- script_full: Full script concept (under 60 seconds)
- hashtags: Array of exactly 4 hashtags

Respond in JSON format only.`

  if (!apiKey) {
    return trends.slice(0, count).map((trend) => ({
